...
pub mod encoding {
    ...
    impl ModRM {
        ...
        pub fn split(&self) -> (ModBits, Register, Register) {
        ...
        pub fn bytes_remaining(&self, sib: Option<Sib>) -> Result<usize, DecodeError> {
            ...
            let displacement = match self.0 {
                ModBits::OO => {
                    ...
                    } else if self.2 == Register::ESP {
                        if sib.is_some() {
                            let sib = sib.expect("Is some due to conditional");
                            if sib.2 == Register::EBP {
                                4 /*disp32*/
                            } else {
                                0 /*no displacement*/
                            }
    ...
    impl Sib {
        ...
        pub fn bytes_remaining(&self, modrm: ModRM) -> usize {
            if self.1 == Register::ESP { 0 } else { 4 }
        }

        pub fn sib(bytes: &[u8], idx: usize) -> Result<Sib, DecodeError> {
            let Some(sib) = bytes.get(idx) else {
    ...
    pub mod operands {
        ...
        impl EffectiveAddress {
            ...
            pub fn from(
                ...
                if modrm.precedes_sib_byte() {
                    ...
                    let displacement_used =
                        !(scale == Scale::One && !use_base_register) && modrm.uses_displacement();
                    ...
                    let ea = match use_base_register {
                        true => {
                            // Form: [base + disp]
                            if index == Register::ESP {
                                EffectiveAddress::BaseDisp { base, displacement }
                            } else {
                                // Form: [index*scale + base + disp]
                                EffectiveAddress::IndexBaseDisp {
                                    index,
                                    scale,
                                    base,
                ...
                let effective_address = match mod_bits {
                    ModBits::OO => EffectiveAddress::BaseDisp {
                        base: rm_bits,
                        displacement: Displacement::None,
                    },
                    ModBits::OI | ModBits::IO => EffectiveAddress::BaseDisp {
                        base: rm_bits,
                        displacement,
                    },
                    ModBits::II => EffectiveAddress::Register { reg: rm_bits },
                };
            ...
            pub fn base_d8(base: Register, displacement: u8) -> EffectiveAddress {
                EffectiveAddress::BaseDisp {
                    base,
                    displacement: Displacement::Abs8(displacement),
                }
            }
            ...
            pub fn base_d32(base: Register, displacement: u32) -> EffectiveAddress {
                EffectiveAddress::BaseDisp {
                    base,
                    displacement: Displacement::Abs32(displacement),
                }
            }
            ...
            fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                ...
                match self {
                    ...
                    BaseDisp { base, displacement } => {
                        write!(f, "[ {base}{} ]", displacement.format_optional_operand())
                    }
                    IndexBaseDisp {
                        index,
                        scale,
        ...
        impl Displacement {
            ...
            pub fn format_optional_operand(&self) -> String {
                if *self == Displacement::None || self.get_inner() == 0 {
                    format!("")
                } else {
                    format!(" + {self}")
                }
            ...
            pub fn disp8(bytes: &[u8], base: usize) -> Result<Displacement, DecodeError> {
                let Some(displacement) = bytes.get(base..base + 1) else {
            ...
            pub fn disp32(bytes: &[u8], base: usize) -> Result<Displacement, DecodeError> {
                let Some(displacement) = bytes.get(base..base + 4) else {
            ...
            pub fn from_relative(
                ...
                debug!(
                    "Attempting to grab range {:?} from bytes {:?}",
                    byte_range, bytes
                );
                let Some(dbytes) = bytes.get(byte_range) else {
                    error!(
                        "Index out of bounds when attempting to get bytes for Relative Displacement."
                    );
                ...
                let displacement = match dbytes.len() {
                    1 => {
                        let byte = <[u8; 1]>::try_from(dbytes)
                            .expect("Displacement length calculation should be correct");
                        Displacement::from_byte_relative(location, opcode_length, &byte)
                    }
                    2 => {
                        let bytes = <[u8; 2]>::try_from(dbytes)
                            .expect("Displacement length calculation should be correct");
                        Displacement::from_word_relative(location, opcode_length, &bytes)
                    }
                    4 => {
                        let bytes = <[u8; 4]>::try_from(dbytes)
                            .expect("Displacement length calculation should be correct");
        ...
        impl TryFrom<&[u8]> for Displacement {
            ...
            fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
                let displacement = match value.len() {
                    ...
                    2 => {
                        let Ok(displacement) = <[u8; 2]>::try_from(value) else {
                    ...
                    4 => {
                        let Ok(displacement) = <[u8; 4]>::try_from(value) else {
                            return Err(DecodeError::DecodeFailure);
                        };
    ...
    pub mod extensions {
        ...
        impl ExtSet {
            ...
            pub fn get_sdigit(&self) -> Option<Extension> {
                self.0.iter().find_map(|ext| {
                    let Ok(ext) = Extension::try_from(*ext) else {
                        return None;
                    };
                    if ext.is_sdigit() { Some(ext) } else { None }
                })
            }
        }
