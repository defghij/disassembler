...
pub mod files {
    ...
    fn file2() {
        ...
        let expected: Vec<&str> = expected_raw.split("\n").collect();
...
pub mod edge_cases {
    use rand::Rng;
    use tracing_test::traced_test; // Import the Rng trait
    ...
    fn random_bytes_as_input() {
        ...
        (0..71)
            .step_by(3) // prime bound with prime step.
            .for_each(|i| {
...
pub mod compendium {
    ...
    fn immediate() {
        let mapping: Vec<(&str, &[u8])> = vec![
            ...
            (
                "00000000: 05 DD CC BB AA     add eax, 0xAABBCCDD",
    ...
    fn displacement() {
        let mapping: Vec<(&str, &[u8])> = vec![
            ...
            (
                "00000000: 0F 84 D9 CC BB AA     jz offset_AABBCCDFh",
                &[0x0F, 0x84, 0xD9, 0xCC, 0xBB, 0xAA],
            ),
            //0:   e8 d8 cc bb aa          call   0xaabbccdd
            (
                "00000000: E8 D8 CC BB AA     call offset_AABBCCDDh",
                &[0xE8, 0xD8, 0xCC, 0xBB, 0xAA],
            ),
    ...
    fn opcode_and_immediate() {
        let mapping: Vec<(&str, &[u8])> = vec![(
            "00000000: BB DD CC BB AA     mov ebx, 0xAABBCCDD",
            &[0xBB, 0xDD, 0xCC, 0xBB, 0xAA],
        )];
        mapping.iter().for_each(|(s, b)| {
            check(s.to_string(), b);
        });
    ...
    fn m_rm() {
        let mapping: Vec<(&str, &[u8])> = vec![
            ...
            ("00000000: FF 14 24     call [ esp ]", &[0xFF, 0x14, 0x24]),
        ];
        mapping.iter().for_each(|(s, b)| {
            check(s.to_string(), b);
        });
    ...
    fn mr_rm_and_reg() {
        let mapping: Vec<(&str, &[u8])> = vec![
            ("00000000: 39 FE     cmp esi, edi", &[0x39, 0xFE]),
            (
    ...
    fn rm_reg_and_rm() {
        let mapping: Vec<(&str, &[u8])> = vec![
            ("00000000: 3B FE     cmp edi, esi", &[0x3B, 0xFE]),
            (
    ...
    fn misc_instructions() {
        let mapping: Vec<(&str, &[u8])> = vec![
            ...
            ("00000000: F2 A7     repne cmpsd", &[0xF2, 0xA7]),
        ];
        mapping.iter().for_each(|(s, b)| {
            check(s.to_string(), b);
        });
